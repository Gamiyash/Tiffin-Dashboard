...
const AddInstruction = () => {
  ...
  const checkForDuplicates = () => {
    ...
    const duplicateInstructions = instructionData.some(
      (instruction) =>
        instruction.title.toLowerCase() === newInstruction.title.trim().toLowerCase() &&
        instruction.details.toLowerCase() === newInstruction.details.trim().toLowerCase()
    );

    const duplicateEditInstructions = instructionData.some(
      (instruction) =>
        instruction.title.toLowerCase() === editInstruction.title.trim().toLowerCase() &&
        instruction.details.toLowerCase() === editInstruction.details.trim().toLowerCase()
    );
    ...
    return duplicateInstructions || duplicateInstructiontitle || duplicateInstructionDetails || duplicateEditInstructions || duplicateEditInstructionDetails || duplicateEditInstructiontitle;
  };
            ...
            {editingIndex === index ? (
              <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleSaveEdit(index)} className="bg-green-500 text-white px-4 py-2 rounded-md">
                    Save
                  </button>
                  <button onClick={() => setEditingIndex(null)} className="bg-gray-400 px-4 py-2 text-white rounded-md">
                    Cancel
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleDelete(instruction._id)} className="text-red-500">
                    <FiTrash2 />
                  </button>
                </div>
              </div>
