...
const ManageTiffin = () => {
    ...
    useEffect(() => {
        const mapMenu = async () => {
            ...
            const response1 = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/tiffin/gamiyash15@gmail.com`);
    ...
    const handleSaveMealDays = async () => {
        try {
            const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/manage_mealdays&Flexidates/gamiyash15@gmail.com`, {
                serviceDays,
                isFlexibleDates,
            });
    ...
    const handleDelete = async (item, type) => {
        ...
        try {
            if (type === "plan") {
                ...
                await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/delete-plan/${item._id}/gamiyash15@gmail.com`);
                setPlans(plans.filter(plan => plan._id !== item._id));
            } else if (type === "mealType") {
                // Call the delete meal type route
                await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/delete-meal-type/${item.mealTypeId}/gamiyash15@gmail.com`);
